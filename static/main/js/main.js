document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typingTextContainer) {
    const fullHTML = typingTextContainer.innerHTML.trim();
    typingTextContainer.innerHTML = '';
    
    let i = 0;
    const speed = 95; // Typing speed in milliseconds per character
    
    function type() {
      if (i < fullHTML.length) {
        if (fullHTML[i] === '<') {
          // If we encounter an HTML tag, skip typing it character by character
          // to avoid rendering raw HTML markup on the screen.
        ...
        // Add class to trigger CSS animations for other elements and cursor fade-out
    ...
    // Add a slight delay before typing starts to make it feel premium
    setTimeout(type, 450);
  }
...
function initTeamMatcherDemo() {
  const categorySelect = document.getElementById('team-category');
  const frequencySelect = document.getElementById('team-frequency');
  const roleSelect = document.getElementById('team-role');
  const sizeSelect = document.getElementById('team-size');
  
  const matchBtn = document.getElementById('team-match-btn');
  const matchResult = document.getElementById('team-match-result');
  
  const matchAvatar = document.getElementById('team-match-avatar');
  const matchName = document.getElementById('team-match-name');
  const matchDept = document.getElementById('team-match-dept');
  const matchScore = document.getElementById('team-match-score');
  
  const compGoal = document.getElementById('comp-bar-goal');
  const compTime = document.getElementById('comp-bar-time');
  const compComplement = document.getElementById('comp-bar-complement');
  const analysisText = document.getElementById('team-match-analysis-text');
  ...
  if (matchBtn && categorySelect && frequencySelect && roleSelect && sizeSelect && matchResult) {
    matchBtn.addEventListener('click', () => {
      ...
      setTimeout(() => {
        let teamName = '해커톤 준비팀 "데브캠퍼스"';
        let leaderName = '팀장: 김민우 (컴퓨터공학과)';
        let teamAvatar = '데';
        
        let goalFit = 95;
        let timeFit = 90;
        let complementFit = 95;
        ...
        const avgScore = Math.round((goalFit + timeFit + complementFit) / 3);
...
function initInteractiveBackground() {
  ...
  let currentY = 300; // Approximate half height of hero
